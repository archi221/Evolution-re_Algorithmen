...
class Agent:
    ...
    def move(self, direction):
        ...
        # Add 1 or -1 to current position depending on direction
        move_x = self.pos_x if direction >= 2 else self.pos_x + (direction * 2 -1)
        move_y = self.pos_y if direction < 2 else self.pos_y + ((direction - 2) * 2 -1)

        def valid_move(x, y):
            return (
                0 <= x < len(self.map)
                and 0 <= y < len(self.map[0])
                and self.map[x][y] != 1
    ...
    def run(self):
        ...
        for i in range(MAX_STEPS):
            output = self.activate_net(self._get_map_env())
            if last_out != None and last_out == output:
                same_direction += 1
            last_out = output
            if self.move(output) == False:
                false_move += 1
        self.set_fitness(false_move, same_direction)          
        return 
